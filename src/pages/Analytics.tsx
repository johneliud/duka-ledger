...
export function Analytics() {
	...
	const getWeeklyData = () => {
		...
		return days.map(day => {
			const daySales = allSales.filter(s => s.created_at.startsWith(day.date));
				...
				<div className="lg:col-span-2 space-y-4">
					<div className="flex gap-2">
						<button
							...
							className={`px-4 py-2 rounded ${
								view === 'week' ? 'bg-primary text-white' : 'bg-surface border border-border text-text'
							}`}
						>
							Last 7 Days
						</button>
						<button
							...
							className={`px-4 py-2 rounded ${
								view === 'month' ? 'bg-primary text-white' : 'bg-surface border border-border text-text'
							}`}
						>
							Last 4 Weeks
						</button>
					...
					<div className="bg-surface border border-border rounded p-6">
						<h2 className="text-lg font-bold text-text mb-4">
							{view === 'week' ? 'Daily Sales (Last 7 Days)' : 'Weekly Sales (Last 4 Weeks)'}
						</h2>
						<ResponsiveContainer width="100%" height={300}>
							<BarChart data={data}>
								...
								<Tooltip
									...
									formatter={(value) => [`KSh ${(value || 0).toLocaleString()}`, 'Sales']}
								/>
						...
						<div className="grid grid-cols-3 gap-4">
							<div>
								...
								<div className="text-2xl font-bold text-primary">
									KSh {data.reduce((sum, d) => sum + d.total, 0).toLocaleString()}
								</div>
							</div>
							<div>
							...
							<div>
								...
								<div className="text-2xl font-bold text-text">
									KSh {Math.round(data.reduce((sum, d) => sum + d.total, 0) / data.length).toLocaleString()}
								</div>
							</div>
						</div>
					</div>
						...
						<div className="space-y-3 text-sm">
							<div>
								...
								<span className="font-medium text-text">
									{data.reduce((max, d) => d.total > max.total ? d : max, data[0]).name}
								</span>
							</div>
							<div>
							...
							<div>
								...
								<span className="font-medium text-text">
									{data.reduce((min, d) => d.total < min.total ? d : min, data[0]).name}
								</span>
							</div>
						</div>
					</div>
